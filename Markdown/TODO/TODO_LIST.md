...
> 当前版本：最近发布基线 Tag 为 `v1.0.8`（Windows 发布版诊断、Codex 子进程窗口与自动发布流程修复），Tag 指向 `chore: release v1.0.8` 发布提交，manifest 版本一致。产品成熟度仍为 **V1.0-alpha**。发布前须确认 Tag、受控 manifest 和安装包版本一致。
...
- [x] 完成 Windows 真实 Tauri 桌面日常冒烟测试。2026-08-17：托盘、Full/Mini/Ball、关闭到托盘、开机自启、置顶、拖拽、多显示器和 DPI 已实测；数据库恢复与升级场景仍按 P0/P1 单独验收。
...
- [x] 实现旧版本 `provider_<name>` 凭据引用到 UUID 引用的幂等迁移。2026-08-17 提交已落地；真实 Rust/Keychain 集成、失败注入和用户提示验收仍见下一项。
- [ ] 验证迁移失败时不会静默丢失凭据，并提示用户重新录入。状态：失败记录保留并写入脱敏日志/迁移状态；待故障注入测试。
- [x] 实现凭据删除失败提示与数据库失败补偿路径。代码在凭据删除失败时保留 Provider，并在数据库删除失败时补偿恢复凭据；真实凭据库故障测试仍未完成。
...
- [x] 实现数据库损坏安全启动与迁移前快照。仅 SQLite `CORRUPT` / `NOTADB` 会保留 `.db`、`-wal`、`-shm` 为唯一 recovery 副本并以安全空库启动；快照使用版本、时间戳和随机标识命名并保留最近 5 份。
- [ ] 完成 P0-DB-01/P0-DB-02 的 Windows 安装版人工复验，以及快照轮转、WAL、目录不可写和锁冲突故障注入；面向用户的手动恢复入口仍未实现。
...
- [ ] 完整 DIY UI。状态：部分完成；当前支持主题、主题色、Widget 显隐、排序、添加、删除和恢复默认，缩放、自由定位、透明度、圆角、字体和颜色编辑仍未实现。
- [ ] V1.0 插件化 Provider。状态：部分完成；`custom` 当前是通用自定义 API 适配器（配置式接入任意 HTTPS 接口），不是真正的动态插件系统。
- [ ] 自动更新。状态：部分完成；安全守卫、版本升级/降级判断和设置 UI 已完成，生产公钥、HTTPS endpoint 和签名 E2E 尚未完成。
- [ ] 测试用例。状态：Provider HTTP Mock 合约测试已完成；62 条手工测试尚未全部执行，仍需补桌面 E2E、视觉回归和最新恢复/升级场景。
...
- 仍未完成：旧凭据迁移与删除补偿的真实环境验收、系统唤醒刷新、诊断备份恢复、真实跨平台发布验证、生产 updater 配置。
- 需要实际运行验证：托盘行为、窗口模式切换、双击/Tooltip/拖动/滚轮、设置恢复、多显示器、真实 Provider、OS 凭据库和安装升级。
- 当前最高优先级：
  1. 统一 Tag、manifest 与安装包版本，并恢复追踪 P0 数据恢复验收清单。
  2. 完成数据库恢复/迁移/凭据补偿的 Windows 安装版和真实凭据库验收。
  3. 完成 updater 签名配置、真实 Provider 验证、跨平台凭据库测试和发布前安全手工检查。
