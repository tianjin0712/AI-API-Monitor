...
> 当前版本：最近发布基线 Tag 为 `v1.0.6`；`origin/master` 另有 4 个未打 Tag 的自定义 API Provider 提交，当前开发主线版本待下一次发布确定。产品成熟度仍为 **V1.0-alpha**。发布前须确认 Tag、三个受控 manifest 和安装包版本一致。
...
- [ ] 测试用例。状态：Provider HTTP Mock 合约测试已完成；50 条手工测试尚未全部执行，仍需补桌面 E2E、视觉回归和最新恢复/升级场景。
