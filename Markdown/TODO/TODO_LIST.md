...
- [ ] 增加数据库恢复功能，并要求用户明确确认。状态：P0-DB-01 代码修复完成、待 Windows 安装版人工复验；仅 SQLite `CORRUPT` / `NOTADB` 会保留 `.db`、`-wal`、`-shm` 为唯一 recovery 副本并以安全空库启动，面向用户的手动恢复入口仍未实现。
- [ ] 在数据库损坏、目录不可写或锁冲突时提供可恢复启动界面。状态：P0-DB-01 代码修复完成、待 Windows 安装版人工复验；锁冲突有限重试且绝不进入 corruption recovery，目录/I/O 等其他启动错误保留原库并明确失败，待桌面故障注入测试。
