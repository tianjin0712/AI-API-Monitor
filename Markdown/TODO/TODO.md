...
> 当前口径：最近发布基线 Tag 为 `v1.0.7`（通用自定义 API Provider、发布流程与打包脚本、数据库恢复加固），Tag 指向 `chore: release v1.0.7` 发布提交，manifest 版本一致。产品成熟度为 V1.0-alpha。代码实现、自动化测试和真实桌面/发布验收必须分别记录；只有后者完成后才能标记为发布完成。

## P0 发布与数据安全

1. 后续版本发布时统一 Git Tag、受控 manifest 和安装包版本；确认版本同步结果可复现（`v1.0.7` 已完成 Tag 与 release commit 一致性修复）。
