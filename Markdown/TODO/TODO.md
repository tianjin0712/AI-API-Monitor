...
> 当前口径：最近发布基线 Tag 为 `v1.0.6`；`master` 在其上另有 22 个未打 Tag 的提交（通用自定义 API Provider、发布流程与打包脚本、数据库恢复加固），manifest 版本已升至 `1.0.7` 但尚未打 Tag，当前开发主线版本待下一次发布确定。产品成熟度为 V1.0-alpha。代码实现、自动化测试和真实桌面/发布验收必须分别记录；只有后者完成后才能标记为发布完成。
