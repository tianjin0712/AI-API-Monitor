...
当前仍不建议将 V0.3 标记为完整完成。布局加载与自动保存存在覆盖用户配置的竞态，主题在设置页切换时不会持久化，汇总组件还会把人民币、美元以及 Codex 额度等不同单位直接相加，展示结果不具备业务意义。按照 `../Project/mission.md` 的完整 DIY UI 要求，缩放、删除/恢复、透明度、圆角、字体和颜色编辑也尚未实现。
...
- 当前 Widget 数据只有 `id/type/visible`，只能排序和隐藏。`../Project/mission.md` 要求的缩放、删除、透明度、圆角、字体和颜色均没有数据模型和交互实现；布局也是单列顺序，不是可自由定位的桌面 Widget 布局。
...
- 位置：`src-tauri/src/providers/codex.rs:1-12,20-27`，`../Project/README.md` 的 V0.1 功能列表。
...
| 费用预测 | ✅ 已实现 | `get_prediction`：近 7 天日均费用 + 当前余额 → 预计剩余天数/耗尽日期（`../Project/mission.md` §13 公式） |
...
- `../Project/mission.md` 中的自由缩放、删除/添加、透明度、圆角、字体、颜色及自由定位仍无数据模型与交互。
...
- 位置：`../Project/README.md:3,33,64,106` 附近。
