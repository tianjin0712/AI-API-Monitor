...
pub fn add_provider(
    ...
    settings::add_provider(&db, &name, &provider_type, &api_url, &api_key)
}
...
pub fn update_provider(
    ...
    settings::update_provider(&db, id, &name, &api_url, api_key.as_deref())
}
...
pub async fn refresh_all(
    ...
    for provider in providers.iter().filter(|p| p.enabled) {
        match fetch_usage(&manager, provider).await {
            ...
            Err(e) => eprintln!("[refresh_all] provider {} error: {e}", provider.name),
        }
    }
    Ok(out)
}
...
pub fn set_refresh_settings(
    ...
    if foreground_secs == 0 || background_secs == 0 {
        return Err(AppError::Invalid("刷新间隔必须大于 0".into()));
    }
